...
contract TempAdjustedPayments is Initializable {
    ...
    function payWorkers() public {
        ...
        uint lastRequestBlockNumber = block.number;
        getEthPriceContract.requestPriceData();

        while(block.number < lastRequestBlockNumber + 1){} // Wait for ETH price to be updated
    ...
    function calculatePayment(address workerAddress) internal returns (uint) {
        ...
        while(block.number < lastRequestBlockNumber + 1){} // Wait for tempFeel to be updated
