...
describe("TestModifyVariable", function () {
  it("should change x to 1337", async function () {
    ...
    const contract = await ModifyVariable.deploy(10, "someString");
  ...
  it("should change x to Modified string", async function () {
    // this line creates an ethers ContractFactory abstraction: https://docs.ethers.org/v5/api/contract/contract-factory/
    const ModifyVariable = await ethers.getContractFactory("ModifyVariable");
    ...
    await contract.modifyToLeet();
    // getter for state variable x
    const newString = await contract.someString();
    assert.equal(newString, "Modified string");
  });
});